...
CLICK_DECLS
ChatterMessage::ChatterMessage() : _active(false), _channel("default") {
}

ChatterMessage::~ChatterMessage() {
}

int ChatterMessage::configure(Vector<String> &conf, ErrorHandler *errh) {   
    bool active=true;
...
Packet *ChatterMessage::simple_action(Packet *p) {
    emit();
    return p;
}
