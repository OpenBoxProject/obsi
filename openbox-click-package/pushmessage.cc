...
int PushMessage::configure(Vector<String> &conf, ErrorHandler *errh) {   
    bool active=true;
...
Packet *PushMessage::simple_action(Packet *p) {
    emit(p);
    return p;
}
void PushMessage::format_message() {
    Json message = Json();
    message.set("type", _type);
    message.set("content", _message);
    _formated_message = message.unparse();
}
void PushMessage::emit(Packet *p) const {
    if (_attach_packet) {
